# =========================================================
# ひとくちフランス語｜長尺（やさしいフランス語の短編ストーリー聞き流し・複数話）
...
Write a short, easy-to-listen original story in French (level: {LEVEL}).
...
def build_video(stories):
    ...
    main_title = stories[0].get("title_ja", "ひとくちフランス語")
    safe = main_title
    for ch in r'\/:*?"<>|':
    ...
    # エンディング
    end_txt = "Merci d'avoir ecoute!"
    a = make_audio_tgt("Merci d'avoir ecoute!", f"a_{idx}.mp3")
...
def upload(youtube, path, title, stories):
    ...
    description = (
        "やさしいフランス語の短いストーリーを集めた、聞き流しフランス語リスニング（現地語＋和訳つき）。\n"
        "作業用・睡眠用・通勤用にどうぞ。\n"
        f"Stories: {titles}\n\n#フランス語 #フランス語リスニング #フランス語学習 #聞き流し #作業用BGM"
    )
    body = {
        "snippet": {
            "title": title[:100],
